 ...
 *
 * Mobile layout: the search box is pinned to the BOTTOM (just above the on-screen
 * keyboard), and the drink results scroll ABOVE it. So while typing, the filtered
 * drinks stay visible above the keyboard and can be tapped without dismissing it.
 *
 * Feedback: after each add we show a green "✓ تمت إضافة …" banner, flash the
 * tapped button green, and keep a per-drink count badge (✓ N). The sheet stays
 * open so several can be added quickly.
 */
  ...
  return (
    <div className="fixed inset-0 z-50">
      ...
      <div className="absolute inset-x-0 bottom-0 max-w-2xl mx-auto bg-coffee-card border-t border-coffee-line
        ...
        {flash && (
          <div className="mx-4 mb-2 shrink-0 bg-emerald-500/15 border border-emerald-500/40 text-emerald-300
                          text-sm font-extrabold rounded-xl px-3 py-2 text-center">
            ✓ تمت إضافة {flash.name}
          </div>
        )}

        {/* Scrollable results (drinks + custom item) */}
        <div className="flex-1 overflow-y-auto px-4">
          ...
          {/* Custom item */}
          <div className="border-t border-coffee-line pt-3 mt-2 pb-3">
        ...
        {/* Search pinned to the BOTTOM — sits just above the keyboard on mobile */}
        <div className="shrink-0 px-4 py-3 border-t border-coffee-line bg-coffee-card">
          <div className="relative">
            <input
              ...
              className="w-full bg-coffee-card2 border border-coffee-line rounded-xl px-3 py-3 text-base
                         placeholder:text-coffee-muted/60 focus:outline-none focus:border-coffee-gold"
            />
            {query && (
              <button
